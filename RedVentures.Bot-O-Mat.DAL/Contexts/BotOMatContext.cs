...
namespace RedVentures.Bot_O_Mat.API.Data
{
    public class BotOMatContext : DbContext
    {
        ...
        public DbSet<Actor> ErrandActors { get; set; }
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<Actor>().HasQueryFilter(p => p.IsActive);
            #endregion
            ...
            while (actorId <= 100)
            {
                ...
                if (random.Next(1, 10) < 5)
                {
                    ...
                    else modelBuilder.Entity<Robot>().HasData(new Robot { ActorType = ActorType.Robot, Id = actorId, Name = Faker.Name.FullName(), Type = (RobotType)RobotTypevalues.GetValue(random.Next(RobotTypevalues.Length)), DestroyedById = lastkillerId, IsActive = false, ModifiedDate = DateTime.Today.AddDays(-random.Next(30)), Image = randomImageBytes });
                ...
                else
                {
                    ...
                    else modelBuilder.Entity<Cyborg>().HasData(new Cyborg { ActorType = ActorType.Cyborg, Id = actorId, Name = Faker.Name.FullName(), Gender = (Gender)Gendervalues.GetValue(random.Next(Gendervalues.Length)), DestroyedById = lastkillerId, IsActive = false, ModifiedDate = DateTime.Today.AddDays(-random.Next(30)), Image = randomImageBytes });
                    for (int y = 0; y < errandcount; y++)
                    {
                        modelBuilder.Entity<Errand>().HasData(new Errand { ActorId = actorId, Id = ++errandId, Status = (ErrandStatus)ErrandStatusvalues.GetValue(random.Next(ErrandStatusvalues.Length)), Type = (ErrandType)ErrandTypevalues.GetValue(random.Next(ErrandTypevalues.Length)), TimeToComplete = random.Next(1000, 25000) });
                    }
